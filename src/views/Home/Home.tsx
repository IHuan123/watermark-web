        ...
        >
          <CardContent>
            ...
            >
              <IconButton
                color="primary"
                sx={{ p: "5px 5px ", color: "#eee" }}
                aria-label="directions"
              >
                {/* <LinkIcon fontSize={"small"} color="inherit" /> */}
                <VideoTag
                  title={data?.platformInfo.name}
                  type={data?.platformInfo.platform}
                />
              </IconButton>
              ...
              <Divider
                sx={{ height: 28, m: 0.5, display: "none" }}
            ...
            <Alert severity="info" sx={{ m: "10px 0 0" }}>
              如果解析的视频无法播放可尝试在浏览器中直接访问!
            </Alert>
          </CardContent>
        </Card>
      </div>
...
const Home: React.FC<any> = () => {
  const [url, setUrl] = useState<string>(
    "https://m.eyepetizer.net/u1/video-detail?video_id=309719"
  );
    ...
    <div className="home-page">
      <div className="form-box">
        <CustomizedInputBase
          ...
          onClear={onClear}
        />
        <Info show={!isUndef(videoInfo)} data={videoInfo} />
      </div>
    </div>
  );
};
